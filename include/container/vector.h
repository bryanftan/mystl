...
public:
  ...
  vector(const vector &rhs)
      ...
      , length_(0)
      , buffer_(static_cast<pointer>(
            ::operator new(sizeof(value_type) * rhs.capacity_))) {
    try {
  ...
  reference operator[](size_type idx) { return buffer_[idx]; }
...
private:
  ...
  template <NoThrowMoveConstructible U> void moveOrCopyAll(vector &dest) {
  ...
  template <typename U> void moveOrCopyAll(vector &dest) {
    // do a copy when resizing
    std::for_each(buffer_, buffer_ + length_,
                  [&dest](reference item) { dest.pushBackInternal(item); });
  }
  ...
  template <typename... Args> void emplaceBackInternal(Args &&...args) {
